...
unsigned Simplex::apply(ILSParameters *params, tChromosomeReal &sol, tFitness &sol_perf, unsigned maxeval) {
   ...
   while (num < maxeval) {
       ...
       if (m_problem->isBetter(newFitness, bestFitness) || newFitness == bestFitness) {
...
void min_dim_distance(const tChromosomeReal &sol, vector<tChromosomeReal> &pop, vector<unsigned> &minind) {
...
void SimplexNeigh::setPopulation(vector<tChromosomeReal> &pop) {
  m_pop = pop;
}
